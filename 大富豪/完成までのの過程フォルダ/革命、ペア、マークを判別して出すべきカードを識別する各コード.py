...
# 下記player2,player3,player4でそれぞれつくらないといけない

#★CPを強くする方法１：革命ができるかどうか-----------------------------------------------------------------------
#ゴールは下記のように、出したい数字とそれをだすときの手札のインデックスを取得する方法。
#革命を出したいときは、そのインデックスで出すように命令がしやすくなるからね。
list_kakumei_onzon_suuji_with_index=[[['09'], [0, 2, 5, 11]]]
#jokerの枚数を調べる
joker_maisuu=0
can_kakumei=False
list_only_num=[]
...
#★CPを強くする方法２：なるべくペアのカードを出すようにする。フィールドカードが０の時、もしくは場に２、３枚出さないといけないときでも、ぎりぎり勝てるカードを出すようにする。ペアのカードがあるかどうか判別.ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
#４枚の場合については革命の関数で拾えてるはずだから２枚～３枚ペアの数を把握しておけば良い。
joker_maisuu=0
can_kakumei=False
list_only_num=[]
onzon_suuji=[]
          ...
          for i in range(len(player2)) :
                ...
                if list_only_num.count(list_only_num[i])==2 :
                      ...
                      if (list_only_num[i] not in  list_tyouhuku_husegu) :
                           ...
                           listB.append(num)
                           #jokerを含めたインデックス.3枚ペア用
                           num.append(num_index_joker)
                           listA.append(onzon_suuji)
                           listA.append(num)
               ...
               #  if list_only_num.count(list_only_num[i])==3 :
                ...
                if list_only_num.count(list_only_num[i])==1 :
...
#--------3枚のペア------------
#joker_index_3maiでjoker２枚と、好きな数字１枚　[2,3]のように抽出される
#list_onzon_suuji_with_index_pea_3maiでjoker１枚と、ペアの数字２枚。[ [["one"],[2,3,4]]  ]のように抽出される.もしくはjokerなしで同じ数字が３枚

#--------２枚のペア-----------
#joker_index_2mai=でjoker１枚、好きな数字１枚 [2]のように抽出される。
#list_onzon_suuji_with_index_pea_2mai　でjoker 無しで純粋に同じ数字のカードを抽出している。
...
print("2枚のペアを出せるその数字とインデックスは",list_onzon_suuji_with_index_pea_2mai)

"""
下記お試しした結果
ジョーカー2枚のインデックスは []
ジョーカー１枚のインデックスは [12]
3枚のペアを出せるその数字とインデックスは [[['11'], [6, 10, 12]], [['04'], [7, 11, 12]]]
2枚のペアを出せるその数字とインデックスは [[[6, 10, 12], ['11']], [[7, 11, 12], ['04']]]
"""

#改善の余地
#jokerが２枚の時、jokerを使わないでのペア２枚、３枚についてもカウントしたい→jokerが２枚なのはほとんどないからMECEに対応しなくていいか。jokerのインデックスだけおさえておけばいい
#jokerが１枚の時、jokerを使わないでの２ペア、３ペアについてもカウントしたい→joker1枚なのは結構ありえるから改善したい→改善した。jokerを使わない、list_onzon_suuji_with_index_pea_2maiとjokerを使ってのペアのjoker_index_2maiを作成
#jokerが０枚の時、２ペア、３ペアをきちんと抽出できるようにしたい。→した
...
#--------------------------------------------------------------------------------------------------------------
#★CPを強くする方法３：なるべく同じマークを出して縛りが発生するように関数を組む
#フィールドカードが１枚の時、かつ縛りが発生してないときのみ作用するようにする。縛り発生してたらそもそも同じマークしか出さないし
field_card=[[["One","♦"]]]
list_daseru_mark=[]
...
"""
フィールドには ♦ が出ています。そのマークで出せる候補の数字とインデックスは [['03', 0], ['07', 4], ['04', 11]] です。
"""
