...
def finish_turn (select_card) :
     ...
     if num_field_card==1 :
          can_num_card=1
          print("次ターンの人は1枚だけだしてください")
     if num_field_card==2 :
          can_num_card=2
          print("次ターンの人は2枚出してください")
     if num_field_card==3 :
          print("次ターンの人は3枚出してください")
          can_num_card=3
     if num_field_card==4 :
          print("次ターンでの人は枚出してください")
          can_num_card=4
          kakumei=kakumei+1 #革命。あとで2%==0かどうかで偶数、奇数判別して2nが革命中とする。
     if num_field_card==5 :
          print("次ターンの人は5枚出してください")
          can_num_card=5
    ...
    #3枚目に出ていた時
     ...
     #    if eleven_back == 1 :
     #        print("■イレブンバック中です")
     #    print("・Cplayer2の手札はあと",len(player2),"枚です。")
     #    print("・Cplayer3の手札はあと",len(player3),"枚です。")
     #    print("・Cplayer4の手札はあと",len(player4),"枚です。")
     # print("")
     # print("-------------------------------------------")
     pass_kosuu=0
...
def player2_check_kakumei() :
      ...
      if len(player2)>=4 :
          ...
          """print("手札の大きさの平均値は",average_tehuda,"です。７以下なら革命起こす")"""
...
def player3_check_kakumei() :
      ...
      if len(player3)>=4 :
          ...
          #手札の大きさの平均値。手札の平均値が７よりちいさかったら革命してしまおう。２が最強だから相対的な数字の大きさにするべきだけど、目をつむっておく。(するとしたら各+2すればよいか？)
          average_tehuda=(ookisa_tehuda/len(list_only_num))
          """print("手札の数字の大きさの平均値は",average_tehuda,"です。７以下なら革命を起こす")"""
          if (can_kakumei==True) and (average_tehuda <= 7) :
                #その手札を出す
                #print(list_kakumei_onzon_suuji_with_index)の部分
                do_kakumei=True
...
#【CPがペアで出せるものを抽出する関数。フィールドにカードが出てないときに処理させる関数】----------------------------------------------------
...
def player2_dasu() : 
 global CP_card_1,CP_card_2,CP_card_3,CP_card_4,CP_card_5,CP_card_6,your_tehuda,can_num_card,shibari,kakumei,eleven_back,player4,pass_kosuu,agatteru_ninnzuu,agatteruka_p1,agatteruka_p2,agatteruka_p3,agatteruka_p4,flag,p1_jyunni,p2_jyunni,p3_jyunni,p4_jyunni,jyunni,p1_agari,p2_agari,p3_agari,p4_agari,eight_nagasi,do_kakumei,my_turn
 flag = False
 sikou_kaisuu=0
...
def player3_dasu() : 
 global CP_card_1,CP_card_2,CP_card_3,CP_card_4,CP_card_5,CP_card_6,your_tehuda,can_num_card,shibari,kakumei,eleven_back,player4,pass_kosuu,agatteru_ninnzuu,agatteruka_p1,agatteruka_p2,agatteruka_p3,agatteruka_p4,flag,p1_jyunni,p2_jyunni,p3_jyunni,p4_jyunni,jyunni,p1_agari,p2_agari,p3_agari,p4_agari,eight_nagasi,do_kakumei,mu_turn
 flag = False
 sikou_kaisuu=0
 #下記関数内でパスが続いて、field_card.clearしたときに、len(field_card)==0が反応して誤作動を起こしてしまう。だからパス→フィールドリセット時にreset=Falseにして誤作動を防ぐ。
 reset=True
