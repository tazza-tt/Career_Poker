...
#ルールの条件定義。毎回手札を出すたびに関数呼び出して、何枚出せるか？・革命中か？・縛りはあるか？・イレブンバック中か？を判断ーーーーーーーーーーーーーーーーーーーーーー
#初期設定
field_card=[]
kakumei=1 #奇数で通常時、偶数で革命中
shibari=0  #０で縛りなし、１で縛りあり
pass_kosuu=0 #後続３人がパスし、パスが３つになったらfield_card.clear()をする
...
#【手札を出す関数（CPのAIはここで調整する）】ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
...
#CPが手札を関数。順番はplayer2→player3→player4の順番になる。
#今回のは適当に枚数に応じて選択するようにして、それが出せる（True）になるまで繰り返す。#ただし、５，６枚出しはなし。関数つくってないから。
